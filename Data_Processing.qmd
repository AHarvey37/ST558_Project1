...
**these steps do not run anymore with the exception of loading tidyverse**
```{r First Step}
#read in tidyverse 
library(tidyverse)
```
...
## Create functions based on data processing steps
...
combine data
```{r combine function}
combine_data_sets <-function(list1, list2){
  combined_country<-bind_rows(list1[1],list2[1])
  combined_noncountry<-bind_rows(list1[2],list2[2])
  return(list(combined_country,combined_noncountry))
}
```

### Test combination
```{r}
combine_data_sets(list1,list2)
```
