...
create_longer_w_year<-function(pass_df){
  passed_df <- pivot_longer(pass_df,
                            ...
                            cols = ends_with("D"))
  passed_df<- passed_df|>
    mutate(year = as.numeric(substr(passed_df$Info,8,9)))
  
  for (i in nrow(passed_df)) {
    #use if greater than 24 as condition as 2024 is our current year
    if(passed_df$year[i] > 24){
      #if true add 19 to observation, change to numeric and store 4 digits as year
      passed_df$year[i] <- as.numeric(paste0("19",passed_df$year[i]))
      }
    else{
      #if true add 20 to observation, change to numeric and store 4 digits as year
      passed_df$year[i] <- as.numeric(paste0("20",passed_df$year[i]))
      }
  }
  passed_df<-passed_df|>
  #change info to equal the 7 character educational code
  mutate(Info = substr(passed_df$Info,1,7))|>
  #arrange Variables for better visual
  select(area_name, STCOU, Info, year, value)
  return(passed_df)
  }
...
split_tibbles<- function(pass_df){
  ...
  #create county data set
  county_df<- pass_df[x,]
  create_state_var(county_df)
  #create non-county dataset
  nonCounty_df<- pass_df[-x,]
  create_division_var(nonCounty_df)
  #check if county dataset and noncounty dataset combine have same number of rows as new_df
  #i.e. check if any rows were lost
  nrow(county_df)+nrow(nonCounty_df) == nrow(pass_df)
  #add class to datasets
  class(county_df)<- c("county", class(county_df))
  class(nonCounty_df)<- c("non-county", class(nonCounty_df))
  #return data sets
...
parse two data sets
```{r parse}
list1<-my_wrapper("https://www4.stat.ncsu.edu/~online/datasets/EDU01a.csv")
list2<-my_wrapper("https://www4.stat.ncsu.edu/~online/datasets/EDU01b.csv")
