...
create_division_var<- function(nonCounty_df){
  ...
  nonCounty_df2<- nonCounty_df|>
    mutate(division = "test")|>
    mutate(division = if_else(area_name %in% division1,
                                "DIVISION 1", "didnt work"
                                # if_else(area_name %in% division2,
                                #         "DIVISION 2",
                                #         if_else(area_name %in% division3,
                                #                 "DIVISION 3",
...
test wrapper
```{r test wrapper}
test<-read.csv("https://www4.stat.ncsu.edu/~online/datasets/EDU01b.csv")
test<-rename_and_select(test)
test<-create_longer_w_year(test)
test<-split_tibbles(test)
test[1]
